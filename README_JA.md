...
ライブセッション画面の **📚 保管庫** ボタンから、独立したデスクトップ形式のセッション保管庫を開けます。Claude Code / Codex のデスクトップクライアントと CLI にあるユーザーセッションをまとめて索引化し、内部サブエージェントのスレッドは除外します。同じプロバイダーでは公式の resume、別プロバイダーへの引き継ぎではローカルの引継ぎ資料を使用します。
