...
一个实时盯着 **Claude Code** 及 **CodeWhale**（开源 Rust 终端 coding agent）等 coding agent 的桌面宠物：它会随 agent 的状态变表情（思考 / 干活 / 等你授权 / 完成庆祝 / 睡觉），把 Claude 说的话弹成气泡，遇到需要授权时让你一键允许 / 拒绝，并在详情面板里给出 **token 计量与花费**、用量趋势、会话列表。
...
**前置条件**
- macOS / Windows（核心功能全平台可用；「去回复」终端聚焦仅 macOS；领地模式仅 macOS）
- Node.js ≥ 18（含 npm）
...
- `npm run test:windows` —— Windows 适配测试（91 项，验证 win32 增量分支正确性）。
- 日志：`~/.octopus/octopus.log`。

### 打包 / 分发

除源码运行外，可打包成各平台原生安装包（用 [electron-builder](https://www.electron.build)）：

```bash
npm install            # 已含 electron-builder devDependency
npm run dist:mac       # macOS: 生成 .dmg + .zip（arm64 + x64）
npm run dist:win       # Windows: 生成 .exe NSIS 安装包 + portable（x64）
npm run dist:linux     # Linux: 生成 .AppImage + .tar.gz（x64）
npm run dist           # 当前平台默认目标
```

产物输出到 `dist/`（已 gitignore）。配置见 `electron-builder.yml`。

**macOS 用户**也可继续用原项目自带的手工脚本（更轻、不依赖 electron-builder）：

```bash
npm run package:mac    # 生成 dist/Octopus.app + Octopus-mac-arm64.zip（需 swiftc/codesign/ditto，仅 macOS）
```

**Windows 用户**：首次启动时桌宠会自动从 `assets/tray@2x.png` 运行时生成 `tray.ico`（缓存到 `app.getPath('userData')`），无需预置 .ico 文件。详见 [WINDOWS.md](WINDOWS.md)。

**Linux 用户**：AppImage 是单文件可执行，下载后 `chmod +x Octopus-0.1.0.AppImage && ./Octopus-0.1.0.AppImage` 即可运行。

### 计量 / 计费

**双 provider 计量**：
- **Claude Code**：数据源本机 `~/.claude/projects/**/*.jsonl`（只读 token 数 / 模型 / 时间戳，**不读内容**）。状态持久化 `~/.octopus/usage.json`（含 90 天日历、游标）。首次启动回填近 95 天历史。
- **CodeWhale**：数据源 `turn_end` hook 事件直带的 `usage` 对象（R2.3，无需扫 transcript）。状态持久化 `~/.octopus/usage-codewhale.json`。详情面板合并显示两 provider 的总计。

**按完整 model id 精确计价**：
- Claude 模型：从 [LiteLLM 公开价目表](https://github.com/BerriAI/litellm) 同步（opus 各代、fable、sonnet 各代各自独立），未同步时回退家族估算。
- DeepSeek 模型：内置官方定价（deepseek-v4-flash $0.14/$0.28、deepseek-v4-pro $0.435/$2.18，含 cache hit/miss 区分）。
- 可用 `~/.octopus/pricing.json` 覆盖（家族键或精确 `models` 映射）：
  ```json
...
test/windows-adapt.js   Windows 适配测试（84 项）
```
...
- Windows focus tracking（「去回复」终端聚焦）：需原生 helper 或 PowerShell，延迟 ~200-500ms，暂未实现。详见 WINDOWS.md。
