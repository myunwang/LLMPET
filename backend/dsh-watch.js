...
//   turn/end aborted|blocked  → TurnAborted(idle) → 「中断」徽标
...
const { decodeFrames } = require('./zstd');
...
function readLogEntries(fp) {
  ...
  if (!size) return [];
  const out = [];
  for (const line of readLogText(fp, size, String(fp).endsWith('.zstd')).split('\n')) {
...
function createDshWatch(deps) {
  ...
  function handleLine(t, obj) {
    ...
    switch (type) {
      ...
      case 'turn/end': {
        ...
        t.turnActive = false;
        t.didWorkThisTurn = false;
        if (kind === 'aborted' || kind === 'blocked') { update(t, 'idle', 'TurnAborted'); break; }
        if (kind === 'error') {
  ...
  function backfill(t, size, mtimeMs) {
    ...
    const text = readLogText(t.fp, size, t.zstd);
    let title = null;
    ...
    for (const line of text.split('\n')) {
      ...
      switch (obj.type) {
        ...
        case 'approval/asked': pending++; break;
        case 'approval/decided': pending = Math.max(0, pending - 1); break;
        default: break;
      }
    }
    ...
    core.seedSession({
      ...
      sourcePid: null,
      headless: false,
      state: pending > 0 ? 'notification' : 'idle',
      createdAt: mtimeMs,
      updatedAt: mtimeMs,
    });
  }
  ...
  function tick() {
    ...
    for (const { fp, size, mtimeMs } of found) {
      ...
      if (!booted) {
        ...
        if (!cursors.has(fp)) cursors.set(fp, { offset: size, carry: '' });
